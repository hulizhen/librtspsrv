...
struct send_buf *create_send_buf(enum data_type type, unsigned int sz)
{
    ...
    if (!sendp) {
        printd("Allocate memory for send buffer failed!\n");
        return NULL;
    }
    sendp->sz = sz;
    sendp->type = type;
...
static ssize_t send_buf_data(struct rtsp_sess *sessp, struct send_buf *sendp)
{
    ...
    if (sessp->intlvd_mode || sendp->type == DATA_TYPE_RTSP_RESP) {
        ...
        if (ns < 0 || ns != sendp->sz) {
            printd(WARNING "send_buf_data ns = %d\n", ns);
        }
    } else {
        switch (sendp->type) {
...
int create_rtsp_sess(int rtsp_sd)
{
    ...
    if (!sessp) {
        printd(ERR "Allocate memory for RTSP session failed!\n");
        goto err;
    }

    /* Register sd handler. */
    if (register_sd_handler(rtsp_sd, handle_rtsp_sd, sessp) < 0) {
        printd(ERR "register rtsp_sd handler failed!\n");
        goto err;
    }
    ...
    if (!sessp->recv_buf) {
        printd("ERR Allocate memory for receive buffer failed!\n");
        goto err;
    }
    ...
    if (!sessp->strm.frm_info.frm_buf) {
        printd("ERR Allocate memory for frame buffer failed!\n");
        goto err;
    }

    /* Initialize the stream information. */
    get_strm_cb(&sessp->strm.start_strm,
                    &sessp->strm.stop_strm,
                    &sessp->strm.get_frm);
