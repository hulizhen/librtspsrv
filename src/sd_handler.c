 ...
 * Description  : Handlers of socket descriptors.
 * Author       : Hu Lizhen
...
int handle_lsn_sd(struct sd_handler *hp, unsigned int event)
{
    ...
    if (rtsp_sd < 0) {
        perrord(ERR "Accept RTSP connection error");
        return -1;
    }
    printd(INFO "Accept one connection from [%s:%d] ^_^\n",
    ...
    if (setsockopt(rtsp_sd, IPPROTO_TCP, TCP_NODELAY,
        ...
        perrord("Set RTSP socket option [TCP_NODELAY] error");
        return -1;
    }
...
static int filter_data_from_rtsp_sd(struct rtsp_sess *sessp)
{
    ...
    if (nr <= 0) {
        if (nr < 0) {
            perrord(ERR "Receive data from rtsp_sd error");
        }
        return -1;
    } else if (nr == RECV_BUF_SZ - offset) {
        printd(WARNING "Receive buffer seems not enough!\n");
        return 0;
    }
...
int register_sd_handler(int sd, sd_handler_proc_t proc, void *arg)
{
    ...
    if (!hp) {
        printd(ERR "Allocate memory for struct sd_handler failed!\n");
        return -1;
    }
