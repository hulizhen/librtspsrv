...
int create_and_bind(unsigned short port)
{
    ...
    if (sd < 0) {
        printd(EMERG "Create rtsp server listening socket error");
        return -1;
    }
    ...
    if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR,
        ...
        printd(ERR "Set listening socket option[SO_REUSEADDR] error");
        goto err;
    }
    if (bind(sd, (struct sockaddr *)&sa, sizeof(sa)) < 0) {
        printd(EMERG "Bind listening sockaddr to local port[%hd] error: %s\n",
               port, strerror(errno));
        goto err;
    }
...
char *encode_base64(const char *src, unsigned int src_sz)
{
    ...
    if (!res) {
        printd("Allocate memory for encode_base64 failed!\n");
        return NULL;
    }
    dst = res;
    left = src_sz;
